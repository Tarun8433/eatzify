...
Last verified: 2026-09-19 (Gym section, ADR-013) · verified by running the commands in the Checks table, not by reading code.
...
| Gym section (ADR-013) | **built end to end; verified against a local API** | `api/src/gym/` (library, plan, workouts, stats) with 1,324 MIT exercises loaded by migration; the app's Gym hub is a pushed screen off Home and You (D-241), not a sixth tab. Guided workout with progression, supersets, timed holds, cardio, effort, records, heatmap and muscle map. Workout energy is a server estimate; Home adds it to the device's burned figure and names the workout share under it (D-246, amending D-242). The server keeps the two fields apart, and nothing computes "kcal left" from either. No exercise media ships: the dataset's GIFs are © Gym visual and need a licence (D-243). Tracker: `plan.md` |
...
| `cd api && npx jest` | 699 passed, 59 suites (gym: progression, records, energy, muscles, `/logs/day` workout kcal) |
...
| `flutter test` | 877 passed (re-run 2026-09-19; +52 for the Gym) |
...
| `SHOT_DIR=/tmp/shots flutter drive --driver=test_driver/integration_test.dart --target=integration_test/gym_flow_test.dart -d <simulator>` | the Gym journey on a device against a local API, a screenshot per step. Needs a signed-in test account passed as `--dart-define=TEST_ACCESS/TEST_REFRESH/TEST_USER` and `--dart-define=API_BASE_URL` |
...
## The Gym's open items (ADR-013)

1. **The backend has to be deployed** before any release build sees the Gym: the app's release URL
   is the live server, and `/gym` only exists there once `main` is pushed and CI deploys. The
   migration loads the 1,324-exercise library on the way up.
2. **Exercise media is not licensed.** The dataset's GIFs are © Gym visual. The app ships without
   them and reads fine (glyphs, a muscle map, written steps); `GYM_MEDIA_BASE_URL` turns them on
   when a licence exists (D-243). The app now *renders* whatever that variable serves — the library
   row's still, and the animation on the exercise sheet and the workout screen — so switching media
   on is a deployment decision, not a code one. The source art is 180 × 180; it is drawn at 220 dp
   rather than full width for that reason. Setting the variable does not create a licence.
3. **Workout energy is an estimate and says so.** It is never added to `energy_burned_kcal` and
   never changes "kcal left" (D-242). Whether a coach should see it is untouched — that would need
   a docs/10 row.
4. **The workout-day reminder follows the weekly plan only**, not a one-off change to a single date.
5. **The starter plan and progression rules have no clinical sign-off** — they are arithmetic on the
   person's own logged numbers, not advice derived from health data, but docs/17 had ruled workouts
   out entirely and ADR-013 reverses that.
