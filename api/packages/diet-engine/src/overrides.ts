...
function allergenTags(input: EngineInput): readonly string[] {
  return input.foodAllergies.flatMap((a) => [
    `allergen:${a}`,
    `may_contain:${a}`,
  ]);
}
...
export function applyOverrides(
  ...
  const rules = [...(pack.overrides ?? [])].sort(
  ...
  for (const rule of rules) {
    ...
    minLowGiCarbFraction = tighterMax(
      minLowGiCarbFraction,
      c.min_low_gi_carb_fraction,
    );
    ...
    maxCarbGPerOccasion = tighterMin(
      maxCarbGPerOccasion,
      c.max_carb_g_per_occasion,
    );
  ...
  if (
    input.conditions.includes('post_surgery') &&
    input.clinicianAttestation === true
  ) {
